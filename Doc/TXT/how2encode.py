...
def main():
    ...
    parser.add_argument( '-t', '--type', action='store', type=str, choices=['hex', 'uni', 'dec', 'fwd', 'html', 'js', 'b64', 'b32', 'b16', 'utf','full'], default='hex', dest='enc', help='Type of Encode strings. Select [hex => Hexadecimal] or '
                                                          '[uni => Unicode] or [dec => Decimal] or [fwd => Fullwidth]' 
                                                          'or [html => escape javascript] or [b64 => base64] or '
                                                          '[utf => UTF-16/32] [full => envery encode]')
    parser.add_argument('--str', action='store', type=str, dest='txt', help='The String Encode/Decode')
    parser.add_argument("--version", action="version", version="%(prog)s 1.1b")
    ...
    try:
        if (enc == 'hex'):
            ...
            msg = CodingUTF16(txt)
            print(color("[+] UTF-16: ", 2, 1) + "%s" % msg.encoded())
            msg = CodingUTF32(txt)
            print(color("[+] UTF-32: ", 2, 1) + "%s" % msg.encoded())
		elif (enc == 'b16'):
